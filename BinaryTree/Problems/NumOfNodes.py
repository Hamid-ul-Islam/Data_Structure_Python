...
def numOfNodes(root):
    if root is None:
        return 0
    leftNodeCount = numOfNodes(root.left) #numOfNodes() function will keep calling for left node untill it's reached to the None. then it will start calling root.right and if right has left node 
