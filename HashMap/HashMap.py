...
class HashMap: #a list containing a linkedlist in each blocks (initialiezed with None first)
    ...
    #removing from linkedlist applies same. [node1, node2, node3, node4, node5]  here prev = node1 nad head = node2.  so deletion of node2 look like : prev.next(node2) =head(node3) and head=head.next(node5)
    def remove(self, key):
        index = self.hash(key)
