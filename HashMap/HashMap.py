...
class HashMap: #a list containing a linkedlist in each blocks (initialiezed with None first)
    ...
    #removing from linkedlist applies same. [node1, node2, node3, node4, node5]  here prev = node2 nad head = node3.  so deletion of node4 look like : prev=head(node3) and head=head.next(node5)
    def remove(self, key):
        index = self.hash(key)
        ...
        while head:
            if head.key == key:
                if prev is None:
                    self.buckets[index] = None #prev is None for the first node and and we have to remove that with None
                    self.count -= 1
                else:
                    prev.next = head.next
                    self.count -= 1
            
            #until head is not not None and head.key != key while loop will continue by prev=head and head = head.next
            prev = head 
            head = head.next
        return None
